...
export default function EditPortfolioItemPage() {
    ...
    const fetchItem = async () => {
        try {
            ...
            // Authorization check
            if (user && item.freelancerId._id !== user.id) {
            ...
            if (item.completionDate) {
                setValue('completionDate', new Date(item.completionDate).toISOString().split('T')[0]);
            }
    ...
    const onSubmit = async (data: FormData) => {
        ...
        setSubmitting(true);
        try {
            await api.put(`/portfolio/${params.slug}`, {
                ...data,
                coverImage,
    ...
    return (
        <div className="max-w-3xl mx-auto p-6">
            <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">{t('editProject')}</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                    ...
                    <textarea {...register('description')} rows={6} className="input-field" placeholder="Describe the project details..." />
